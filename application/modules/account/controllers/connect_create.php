 ...
 */
class Connect_create extends BaseController {
    ...
    function __construct()
    {
		...
		$this->load->model(array('account/account_details_model', 'account/account_facebook_model', 'account/account_twitter_model', 'account/account_openid_model'));
	...
	function index()
	{
		...
		$this->data['connect_create'] = $this->session->userdata('connect_create');
		...
		if ($this->form_validation->run()) 
		{
			...
			{
				$this->data['connect_create_username_error'] = lang('connect_create_username_taken');
			}
			// Check if email already exist
			elseif ($this->email_check($this->input->post('connect_create_email')) === TRUE)
			{
				$this->data['connect_create_email_error'] = lang('connect_create_email_exist');
			}
			else
			{
				...
				// Add user details
				$this->account_details_model->update($user_id, $this->data['connect_create'][1]);
				
				// Connect third party account to user
				switch($this->data['connect_create'][0]['provider'])
				{
					case 'facebook': $this->account_facebook_model->insert($user_id, $this->data['connect_create'][0]['provider_id']); break;
					case 'twitter': $this->account_twitter_model->insert($user_id, $this->data['connect_create'][0]['provider_id'], $this->data['connect_create'][0]['token'], $this->data['connect_create'][0]['secret']); break;
					case 'openid': $this->account_openid_model->insert($this->data['connect_create'][0]['provider_id'], $user_id); break;
				}
		...
		$this->load->view('connect_create', $this->data);
	}
